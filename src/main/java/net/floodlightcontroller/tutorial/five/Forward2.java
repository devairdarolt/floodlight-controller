...
public class Forward2 implements IFloodlightModule, IOFMessageListener, IControllerCompletionListener {
	protected static Logger log = LoggerFactory.getLogger(Forward2.class);
	...
	private void writeFlowMod(IOFSwitch sw, OFFlowModCommand command, OFBufferId bufferId,
		...
		fmb.setMatch(match);
		fmb.setCookie((U64.of(Forward2.LEARNING_SWITCH_COOKIE)));
		fmb.setIdleTimeout(Forward2.FLOWMOD_DEFAULT_IDLE_TIMEOUT);
		fmb.setHardTimeout(Forward2.FLOWMOD_DEFAULT_HARD_TIMEOUT);
		fmb.setPriority(Forward2.FLOWMOD_PRIORITY);
	...
	private Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		if (outPort == null) {
			...
			// XXX For Forward2 this doesn't do much. The sourceMac is removed
	...
	private Command processFlowRemovedMessage(IOFSwitch sw, OFFlowRemoved flowRemovedMessage) {
		if (!flowRemovedMessage.getCookie().equals(U64.of(Forward2.LEARNING_SWITCH_COOKIE))) {
			return Command.CONTINUE;
		}
		if (log.isTraceEnabled()) {
	...
	public void startUp(FloodlightModuleContext context) {
		...
		counterPacketOut = debugCounterService.registerCounter(this.getName(), "packet-outs-written", "Packet outs written to switches by Forward2", MetaData.WARN);
	}
